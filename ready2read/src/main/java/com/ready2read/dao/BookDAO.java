...
public class BookDAO {
    ...
    /**
     * Returns one page of all books sorted alphabetically by title.
     * OFFSET = (page - 1) * pageSize skips the correct number of rows for the requested page.
     */
    ...
    /** Same pagination pattern as getAllBooks, filtered to a specific genre. */
    ...
    /** Fetches a single book by primary key. Returns null if not found. */
    ...
    /** Inserts a new book row. BookID is auto-generated by the database. */
    ...
    /** Updates all editable fields for an existing book identified by BookID. */
    ...
    /** Permanently removes a book row by primary key. */
    ...
    /**
     * Computes the average rating for a book from the Reviews table.
     * AVG() returns NULL if there are no reviews
     * Check wasNull() distinguishes "no reviews" from a real 0.0 average.
     */
    ...
    /**
     * Returns the total number of books —-> used by the caller to calculate how
     * many pages exist for the paginated catalog view.
     */
    public int getTotalBookCount() {
        String sql = "SELECT COUNT(*) FROM Books";
        // No parameters to bind, so ResultSet can be opened directly in try-with-resources.
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    /** Same as getTotalBookCount but only counts books in the given genre. */
    ...
    /**
     * Checks whether an ISBN is already in use by another book.
     * excludeBookID is the book currently being edited, so its own ISBN
     * doesn't trigger a false duplicate conflict during an update.
     */
    ...
    /** Returns all unique genres in the catalog, used for the genre filter dropdown. */
    public List<String> getAllGenres() {
        List<String> genres = new ArrayList<>();
        String sql = "SELECT DISTINCT Genre FROM Books WHERE Genre IS NOT NULL ORDER BY Genre";
        // No parameters; ResultSet opened directly in try-with-resources.
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
    ...
    /**
     * Full-text search across Title, Author, and ISBN with optional genre filter.
     * Builds the query dynamically since genre is optional & tracks placeholder 
     * positions so numbering stays correct.
     */
    public List<Book> searchBooks(String query, String genre, int page, int pageSize) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            // Wrap the query in % wildcards for a LIKE contains-match.
            String search = "%" + query + "%";
            int index = 1;
            ...
            // Bind the same search term to each of the three LIKE clauses.
    ...
    /**
     * Returns the total row count for a search query, used to calculate pagination
     * for search results. Uses the same dynamic SQL approach as searchBooks.
     */
